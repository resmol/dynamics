...
def random_maxwell(atom_i, T):
    ...
    # Gauss distribution with sigma standart deviation and mean value.
    vel = rnd.gauss(mean, sigma)
...
def hess_check(Hessii):
    for i in Hessii:
        if Hessii < 0.:
            raise Hessian_problem(
                "THERE IS A PROBLEM WITH THE HESSIAN MATRIX IN INTERNAL COORDINATES")
