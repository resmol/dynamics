...
def cluster(ndim,estado,file):
    if estado == 'S0':
        os.system("./cluster")
    if estado == 'T1':
        os.system("./T1_cluster")

    mtx_int = open(file)
    Hess = np.empty((ndim,ndim), dtype=float)
    for i in xrange(ndim):
        blank = mtx_int.readline()
        for j in xrange(ndim):
                Hess[i,j] = mtx_int.readline()
    mtx_int.close()
    return Hess
...
def maxwell_vel(A,T):
    vel = A.vel.x
    a1 =sqrt(A.mass/(2*pi*T*kb))
    a2 = (A.mass*vel*vel)/(2*T*kb)
    prob = a1*exp(-a2)
            
    return prob/1.e3, vel*1.e3

def random_maxwell(atom_i,T):
    sigma = sqrt(T*kb/atom_i.mass) # Standart deviation 
    mean = 0. # Mean value of the distribution
    vel = rnd.gauss(mean,sigma) # Gauss distribution with sigma standart deviation and mean value.
 
    return vel

def gauss_dis(var,mean,x):
    a1 = 1/sqrt(2*pi*var)
    a2 = ((x-mean)**2.)/(2.*var)
    P = a1*exp(-a2)
...
def read_grad(dimen,file):
    vector = open(file)
    gradiente = np.empty((dimen), dtype=float)
    for i in xrange(dimen):
        gradiente[i] = vector.readline()
    vector.close()
    return gradiente 

def initial_data(fileS0,state):
    cord = list()
    data = open(fileS0)
    ...
    print 'Number of atoms', int(lines[n1+5])/3
    numat =int(lines[n1+5])/3
    n2 = n1+6
    lista_cord = (lines[n2:n2+(3*numat)])
    cord = [float(w) for w in lista_cord]
  
    if state == 'state1':
        #reading atom type
        nN1 = lines.index('Nuclear')
        nN2 = nN1 + 5
        N_charges = [float(m) for m in lines[nN2:nN2+numat]]
        numbers = list(range(numat))
        typ = atom_type(N_charges)
        symb =[typ[m]+str(numbers[m]) for m in range(len(typ))]
       
        # Reading the Real atomic weights
        n5 = lines.index('Real')
        n6 = n5 + 6
        m_uma = [float(w)*au for w in lines[n6:n6+numat]]
        mass =  np.array(m_uma,dtype=float)
        data.close()
   
    # Reading the total energy
    n8 = lines.index('Total')
    n9 = n8 + 3
    energy = float(lines[n9])
   
    #reading of the Gradient in Cartesian Cordinates
    n3 = lines.index('Gradient')
    n4 = n3 + 4
    grad =(lines[n4:n4+(3*numat)])
    gx = np.array([float(x) for x in grad],dtype=float)
    ...
    ncart = 3*numat
    Hess = np.empty((ncart,ncart), dtype = float)
    for i in range(1,ncart+1):
        for j in range(1,i+1):
            n = (i*i - i)/2 + j
            Hess[i-1,j-1] =float(vector[n-1])
            Hess[j-1,i-1] = Hess[i-1,j-1]
    data.close()

    if state == 'state1':
        return cord, numat, typ, symb, mass, energy, gx, Hess
    if state == 'state2':
        return energy, gx, Hess        

## Checking functions
def hess_check(Hessii):
    for i in Hessii:
        if Hessii < 0.:
            raise Hessian_problem("THERE IS A PROBLEM WITH THE HESSIAN MATRIX IN INTERNAL COORDINATES")
...
def MOVEA(mol,dt):
    ...
    for i in xrange(len(mol)):
        mol[i].cart.x = mol[i].cart.x + dt * mol[i].vel.x + dtsq2 * mol[i].force.x / mol[i].mass
        mol[i].cart.y = mol[i].cart.y + dt * mol[i].vel.y + dtsq2 * mol[i].force.y / mol[i].mass
        mol[i].cart.z = mol[i].cart.z + dt * mol[i].vel.z + dtsq2 * mol[i].force.z / mol[i].mass
...
def bath(mol,dt,thermo,Ek,T,numat):
    dt2=  dt*.5; dt4 = dt*0.25 ; dt8 = dt*0.125
...
def nose_hoover_1(mol,dt,thermo,T,numat):
    ...
    Ekinetic = sum([0.5*mol[j].mass*cuad(mol[j].vel,mol[j].vel) for j in range(numat)])
...
def Inertia_matrix(atom):
    ...
    I_matrix[0,0] =  sum([atom[n].mass * (atom[n].cart.y**2 +  atom[n].cart.z**2) for n in range(numat)]) 
    I_matrix[0,1] = -sum([atom[n].mass*atom[n].cart.x*atom[n].cart.y for n in range(numat)])
    I_matrix[0,2] = -sum([atom[n].mass*atom[n].cart.x*atom[n].cart.z for n in range(numat)])

    I_matrix[1,0] = I_matrix[0,1]
    I_matrix[1,1] = sum([atom[n].mass * (atom[n].cart.x**2 +  atom[n].cart.z**2) for n in range(numat)]) 
    I_matrix[1,2] = -sum([atom[n].mass*atom[n].cart.y*atom[n].cart.z for n in range(numat)])

    I_matrix[2,0] = I_matrix[0,2]
    I_matrix[2,1] = I_matrix[1,2]
    I_matrix[2,2] = sum([atom[n].mass * (atom[n].cart.x**2 +  atom[n].cart.y**2) for n in range(numat)]) 
...
def derv_ang(A,B,C):
    ...
    r1 = sqrt(dif(A,B)); r2 = sqrt(dif(B,C))
    teta = angulos(A,B,C)
    ...
    for i in range(3):
        a1 =d1[i]/(r1*r2*sin(teta)) ; a2 = (1.0/tan(teta))*(d2[i]/(r1*r1))
        t1.append(a1+a2)
        b1=(1.0/tan(teta))*(((-1*d2[i])/(r1*r1))+(d1[i]/(r2*r2))); b2 = d3[i]/(r1*r2*sin(teta))
        t2.append(b1-b2)
        c1=(-1*d2[i])/(r1*r2*sin(teta)); c2 = (1.0/tan(teta))*((-1*d1[i])/(r2*r2))
        t3.append(c1+c2)
    return (t1,t2,t3)
...
def seg_derv_bond(A,B,m,n):
    r = sqrt(dif(A,B)); d_ij = kronecker(m,n)
    Ax = attr_val(A); Bx = attr_val(B)
    derv = (-1./r)*(((Ax[m]-Bx[m])*(Ax[n]-Bx[n])/(r*r)) - d_ij)

    return derv

def seg_derv_ang(A,B,C,DA,DB,DC,i,j):
    Ax = attr_val(A); Bx = attr_val(B); Cx = attr_val(C)
    r1 = sqrt(dif(A,B)); r2 = sqrt(dif(C,B)); d_ij = kronecker(i,j)
    teta = angulos(A,B,C)
    a1 = cos(teta); a2 = sin(teta); a3 = tan(teta)
...
def numerical_derv_AA(i,j,A,B,C,dx):
    ...
    rmb_D1 = cross(rab_D1,rcb); RMB_D1 = sqrt(sum([r*r for r in rmb_D1]))
    rmb_D2 = cross(rab_D2,rcb); RMB_D2 = sqrt(sum([r*r for r in rmb_D2]))
    rmb_D3 = cross(rab_D3,rcb); RMB_D3 = sqrt(sum([r*r for r in rmb_D3]))
    rmb_D4 = cross(rab_D4,rcb); RMB_D4 = sqrt(sum([r*r for r in rmb_D4]))
...
def numerical_derv_AB(i,j,A,B,C,dx):
    ...
    rcb_D1 = [C.x -B1.x, C.y -B1.y, C.z -B1.z] ; RCB_D1 = sqrt(sum([r*r for r in rcb_D1]))
    rcb_D2 = [C.x -B2.x, C.y -B2.y, C.z -B2.z] ; RCB_D2 = sqrt(sum([r*r for r in rcb_D2]))
    rcb_D3 = [C.x -B3.x, C.y -B3.y, C.z -B3.z] ; RCB_D3 = sqrt(sum([r*r for r in rcb_D3]))
    rcb_D4 = [C.x -B4.x, C.y -B4.y, C.z -B4.z] ; RCB_D4 = sqrt(sum([r*r for r in rcb_D4]))

    rmb_D1 = cross(rab_D1,rcb_D1); RMB_D1 = sqrt(sum([r*r for r in rmb_D1]))
    rmb_D2 = cross(rab_D2,rcb_D2); RMB_D2 = sqrt(sum([r*r for r in rmb_D2]))
    rmb_D3 = cross(rab_D3,rcb_D3); RMB_D3 = sqrt(sum([r*r for r in rmb_D3]))
    rmb_D4 = cross(rab_D4,rcb_D4); RMB_D4 = sqrt(sum([r*r for r in rmb_D4]))
...
def numerical_derv_AC(i,j,A,B,C,dx):
    ...
    rcb_D1 = [C1.x -B.x, C1.y -B.y, C1.z -B.z] ; RCB_D1 = sqrt(sum([r*r for r in rcb_D1]))
    rcb_D2 = [C2.x -B.x, C2.y -B.y, C2.z -B.z] ; RCB_D2 = sqrt(sum([r*r for r in rcb_D2]))
    rcb_D3 = [C3.x -B.x, C3.y -B.y, C3.z -B.z] ; RCB_D3 = sqrt(sum([r*r for r in rcb_D3]))
    rcb_D4 = [C4.x -B.x, C4.y -B.y, C4.z -B.z] ; RCB_D4 = sqrt(sum([r*r for r in rcb_D4]))

    rmb_D1 = cross(rab,rcb_D1); RMB_D1 = sqrt(sum([r*r for r in rmb_D1]))
    rmb_D2 = cross(rab,rcb_D2); RMB_D2 = sqrt(sum([r*r for r in rmb_D2]))
    rmb_D3 = cross(rab,rcb_D3); RMB_D3 = sqrt(sum([r*r for r in rmb_D3]))
    rmb_D4 = cross(rab,rcb_D4); RMB_D4 = sqrt(sum([r*r for r in rmb_D4]))
...
def numerical_derv_BC(i,j,A,B,C,D,dx):
    ...
    rcd_D1 = [C1.x -D.x, C1.y -D.y, C1.z -D.z] ; RCD_D1 = sqrt(sum([r*r for r in rcd_D1]))
    rcd_D2 = [C2.x -D.x, C2.y -D.y, C2.z -D.z] ; RCD_D2 = sqrt(sum([r*r for r in rcd_D2]))
    rcd_D3 = [C3.x -D.x, C3.y -D.y, C3.z -D.z] ; RCD_D3 = sqrt(sum([r*r for r in rcd_D3]))
    rcd_D4 = [C4.x -D.x, C4.y -D.y, C4.z -D.z] ; RCD_D4 = sqrt(sum([r*r for r in rcd_D4]))
...
def numerical_derv_BD(i,j,A,B,C,D,dx):
    ...
    rcd_D1 = [C.x -d1.x, C.y -d1.y, C.z -d1.z] ; RCD_D1 = sqrt(sum([r*r for r in rcd_D1]))
    rcd_D2 = [C.x -d2.x, C.y -d2.y, C.z -d2.z] ; RCD_D2 = sqrt(sum([r*r for r in rcd_D2]))
    rcd_D3 = [C.x -d3.x, C.y -d3.y, C.z -d3.z] ; RCD_D3 = sqrt(sum([r*r for r in rcd_D3]))
    rcd_D4 = [C.x -d4.x, C.y -d4.y, C.z -d4.z] ; RCD_D4 = sqrt(sum([r*r for r in rcd_D4]))
...
def numerical_derv_DB(i,j,B,C,D,dx):
    ...
    rnc_D1 = cross(rcb_D1,rcd); RNC_D1 = sqrt(sum([r*r for r in rnc_D1]))
    rnc_D2 = cross(rcb_D2,rcd); RNC_D2 = sqrt(sum([r*r for r in rnc_D2]))
    rnc_D3 = cross(rcb_D3,rcd); RNC_D3 = sqrt(sum([r*r for r in rnc_D3]))
    rnc_D4 = cross(rcb_D4,rcd); RNC_D4 = sqrt(sum([r*r for r in rnc_D4]))
...
def numerical_derv_DC(i,j,B,C,D,dx):
    ...
    rnc_D1 = cross(rcb_D1,rcd_D1); RNC_D1 = sqrt(sum([r*r for r in rnc_D1]))
    rnc_D2 = cross(rcb_D2,rcd_D2); RNC_D2 = sqrt(sum([r*r for r in rnc_D2]))
    rnc_D3 = cross(rcb_D3,rcd_D3); RNC_D3 = sqrt(sum([r*r for r in rnc_D3]))
    rnc_D4 = cross(rcb_D4,rcd_D4); RNC_D4 = sqrt(sum([r*r for r in rnc_D4]))
...
def numerical_derv_DD(i,j,B,C,D,dx):
    ...
    rnc_D1 = cross(rcb,rcd_D1); RNC_D1 = sqrt(sum([r*r for r in rnc_D1]))
    rnc_D2 = cross(rcb,rcd_D2); RNC_D2 = sqrt(sum([r*r for r in rnc_D2]))
    rnc_D3 = cross(rcb,rcd_D3); RNC_D3 = sqrt(sum([r*r for r in rnc_D3]))
    rnc_D4 = cross(rcb,rcd_D4); RNC_D4 = sqrt(sum([r*r for r in rnc_D4]))
...
def segunda_wilson(mol,ndim,numat,bond,ang,dih):
    ...
    for k in range(len(bond)):
        m1 = bond[k][0]; n1=3*(m1-1)
        m2 = bond[k][1]; n2=3*(m2-1)
        for i in range(3):
            for j in range(3):
                seg_wilson[k,n1+i,n1+j] = seg_derv_bond(mol[m1-1].cart,mol[m2-1].cart,i,j)
                seg_wilson[k,n1+i,n2+j] = -1 * seg_derv_bond(mol[m1-1].cart,mol[m2-1].cart,i,j)
                seg_wilson[k,n2+j,n1+i] = seg_wilson[k,n1+i,n2+j]
                seg_wilson[k,n2+j,n2+i] = seg_wilson[k,n1+i,n1+j]
 
    for k in range(len(ang)):
        m1 = ang[k][0]; m2 = ang[k][1]; m3 = ang[k][2] ; nq = k + len(bond)
        n1 = 3*(m1-1);  n2 = 3*(m2-1);  n3 = 3*(m3-1)
        DA,DB,DC =  derv_ang(mol[m1-1].cart,mol[m2-1].cart,mol[m3-1].cart)
        for i in range(3):
            for j in range(3):
                part1, part2, part3 = seg_derv_ang(mol[m1-1].cart,mol[m2-1].cart,mol[m3-1].cart,DA,DB,DC,i,j)
                
                seg_wilson[nq,n1+i,n1+j],seg_wilson[nq,n1+i,n2+j],seg_wilson[nq,n1+i,n3+j]= part1
                seg_wilson[nq,n2+i,n1+j],seg_wilson[nq,n2+i,n2+j],seg_wilson[nq,n2+i,n3+j]= part2
                seg_wilson[nq,n3+i,n1+j],seg_wilson[nq,n3+i,n2+j],seg_wilson[nq,n3+i,n3+j]= part3

    for k in range(len(dih)):
        m1 = dih[k][0]; m2 = dih[k][1]; m3 = dih[k][2]; m4 = dih[k][3]; nq = k+len(bond)+len(ang)
        n1 = 3*(m1-1); n2 = 3*(m2-1); n3 =3*(m3-1); n4 = 3*(m4-1)

        A,B,C,D = mol[m1-1].cart, mol[m2-1].cart, mol[m3-1].cart, mol[m4-1].cart
        RAB =sqrt(dif(A,B)); RCB = sqrt(dif(C,B)); RDC = sqrt(dif(C,D))
        ...
        for i in range(3):
            for j in range(3):
                seg_wilson[nq,n1+i,n1+j] = numerical_derv_AA(i,j,A,B,C,dx)
                seg_wilson[nq,n1+i,n2+j] = numerical_derv_AB(i,j,A,B,C,dx)
                seg_wilson[nq,n1+i,n3+j] = numerical_derv_AC(i,j,A,B,C,dx)
                seg_wilson[nq,n1+i,n4+j] = 0.
             
                seg_wilson[nq,n2+j,n1+i] = numerical_derv_BA(j,i,A,B,C,D,dx)
                seg_wilson[nq,n2+i,n2+j] = numerical_derv_BB(i,j,A,B,C,D,dx)
                seg_wilson[nq,n2+i,n3+j] = numerical_derv_BC(i,j,A,B,C,D,dx)
                seg_wilson[nq,n2+i,n4+j] = numerical_derv_BD(i,j,A,B,C,D,dx)
             
                seg_wilson[nq,n3+j,n1+i] = numerical_derv_CA(j,i,A,B,C,D,dx)
                seg_wilson[nq,n3+j,n2+i] = numerical_derv_CB(j,i,A,B,C,D,dx)
                seg_wilson[nq,n3+i,n3+j] = numerical_derv_CC(i,j,A,B,C,D,dx)
                seg_wilson[nq,n3+i,n4+j] = numerical_derv_CD(i,j,A,B,C,D,dx)
             
                seg_wilson[nq,n4+j,n1+i] = 0.
                seg_wilson[nq,n4+j,n2+i] = numerical_derv_DB(j,i,B,C,D,dx)
                seg_wilson[nq,n4+j,n3+i] = numerical_derv_DC(j,i,B,C,D,dx)
                seg_wilson[nq,n4+i,n4+j] = numerical_derv_DD(i,j,B,C,D,dx)
...
def mtx_conect(file):
    ...
    internas.write('%d' % len(A)); internas.write('\n')
    for i in xrange(len(A)):
        internas.write('%d  %d' % (A[i][0], A[i][1])); internas.write('\n') 
    internas.write('%d' % len(ang)); internas.write('\n')
    for j in xrange(len(ang)):
        internas.write('%d  %d  %d' % (ang[j][0], ang[j][1], ang[j][2])); internas.write('\n')
    internas.write('%d' % len(dih)); internas.write('\n')
    for k in xrange(len(dih)):
        internas.write('%d  %d  %d  %d' % (dih[k][0], dih[k][1], dih[k][2], dih[k][3])); internas.write('\n')
...
def internas_resize(conex_bond,conex_ang,conex_dih):
    ...
    internas = open('internas.dat','w')
    internas.write('%d' % len(conex_bond)); internas.write('\n')
    for i in xrange(len(conex_bond)):
        internas.write('%d  %d' % (conex_bond[i][0], conex_bond[i][1])); internas.write('\n') 
    internas.write('%d' % len(conex_ang)); internas.write('\n')
    for j in xrange(len(conex_ang)):
        internas.write('%d  %d  %d' % (conex_ang[j][0], conex_ang[j][1], conex_ang[j][2])); internas.write('\n')
    internas.write('%d' % len(conex_dih)); internas.write('\n')
    for k in xrange(len(conex_dih)):
        internas.write('%d  %d  %d  %d' % (conex_dih[k][0],conex_dih[k][1],conex_dih[k][2],conex_dih[k][3])); internas.write('\n')
    
def write_freq(freq,nbond,nang,ndih,(floor,roof),m):
    nombre = 'histograma' + '_' + str(float(floor)) + '_' +str(float(roof)) + '.out'
    histo = open(nombre,'w')
    histo.write(' the interval of energy is [%g,%g] in kcal/mol' % (floor,roof)); histo.write('\n')
    ...
    for d in freq:
        if cont < nbond:
            histo.write('bond number %d' % cont ); histo.write('\n')
            lista = [ key for key in d]
            lista.sort()
            for val in lista:
                mid = (val[0]+val[1])/2.
                histo.write( '%g %d' % (mid,freq[cont][val])); histo.write('\n')

        histo.write('\n')
        if nbond <= cont and cont < nang+nbond:
            number = 1+cont-nbond
            histo.write('angle number %d' % (number) ); histo.write('\n')
            lista = [ key for key in d]
            lista.sort()
            for val in lista:
                mid = (val[0]+val[1])/2.
        ...
        if nang+nbond <= cont:
            ...
            histo.write('dihedral number %d' % (number) ); histo.write('\n')
            lista = [ key for key in d]
            lista.sort()
            for val in lista:
                mid = (val[0]+val[1])/2.
